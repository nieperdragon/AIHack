...
export class PIMPage extends BasePage {
    ...
    constructor(page: Page) {
        ...
        await this.page.waitForURL(/\/pim\/viewPersonalDetails\/empNumber\/\d+/, { timeout: 20000 });
    ...
    async getSearchResults(): Promise<Locator> {
        await this.page.waitForLoadState('networkidle');
        // Check for "No Records Found" message using a specific locator
        const noRecordsLocator = this.page.locator('span.oxd-text.oxd-text--span', { hasText: 'No Records Found' });
    ...
    async selectFirstEmployeeRowCheckbox(): Promise<void> {
        // Click the visible checkbox span in the first row (not the input directly), with force
        await this.employeeList.table.rows.first().locator('span.oxd-checkbox-input').click({ force: true });
    }

    async viewLastEmployeeDetails(): Promise<void> {
        // Wait for the employee list table to be visible
        await this.employeeList.table.rows.first().waitFor({ state: 'visible', timeout: 10000 });
        const rowCount = await this.employeeList.table.rows.count();
        if (rowCount === 0) {
            throw new Error('No employees found in the list.');
        }
        // Click the last row to view details
        await this.employeeList.table.rows.nth(rowCount - 1).click();
